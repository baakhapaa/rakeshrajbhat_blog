    ...
    <div class="max-w-7xl mx-auto h-20 px-6 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center space-x-8 text-sm font-medium tracking-wide text-gray-300">
            <a href="{{ route('home') }}#home" 
               class="nav-link transition-colors {{ request()->routeIs('home') ? 'active text-[#D4AF37] border-b-2 border-[#D4AF37] pb-1' : 'hover:text-[#D4AF37]' }}">
                Home
            </a>
        ...
        @if(request()->routeIs('blog') || request()->routeIs('blog.show'))
            ...
            @auth
                <div class="relative group">
                    {{-- User Menu Trigger --}}
                    <button type="button" 
                            class="flex items-center gap-2 text-white hover:text-[#D4AF37] transition" 
                            aria-haspopup="true" 
                            aria-expanded="false">
                    ...
                    {{-- Dropdown Menu --}}
                    <div class="absolute right-0 top-full mt-3 w-48 bg-white rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible focus-within:opacity-100 focus-within:visible transition-all duration-200 z-50">
                        <div class="px-4 py-3 border-b">
                            <p class="text-sm font-semibold text-gray-800">{{ Auth::user()->name }}</p>
                            <p class="text-xs text-gray-500">{{ Auth::user()->email }}</p>
                        </div>

                        <a href="{{ route('profile') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-[#D4AF37] transition">
                        ...
                        <a href="{{ route('settings') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-[#D4AF37] transition">
                            <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
