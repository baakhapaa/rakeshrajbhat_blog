...
@section('title', 'Edit Research · Admin')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
            ...
            <p class="text-gray-400 text-sm mt-1">Update research item details</p>
        </div>
    ...
    <form action="{{ route('admin.research.update', $research) }}" method="POST" enctype="multipart/form-data" class="bg-[#1a1f26] rounded-xl border border-white/5 p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" id="title" name="title" value="{{ old('title', $research->title) }}" required
            ...
            <div>
                ...
                <select id="category" name="category" required class="w-full px-4 py-2.5 bg-[#0b0e12] border border-white/10 rounded-lg text-white focus:border-[#D4AF37] focus:outline-none transition">
                    ...
                    <option value="Vision" {{ old('category', $research->category) == 'Vision' ? 'selected' : '' }}>Vision</option>
                    <option value="Research Papers" {{ old('category', $research->category) == 'Research Papers' ? 'selected' : '' }}>Research Papers</option>
                    <option value="Media" {{ old('category', $research->category) == 'Media' ? 'selected' : '' }}>Media</option>
                </select>
                @error('category')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" id="order" name="order" value="{{ old('order', $research->order) }}" min="0"
                    class="w-full px-4 py-2.5 bg-[#0b0e12] border border-white/10 rounded-lg text-white placeholder:text-gray-500 focus:border-[#D4AF37] focus:outline-none transition">
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="description" name="description" rows="3" required
                    class="w-full px-4 py-2.5 bg-[#0b0e12] border border-white/10 rounded-lg text-white placeholder:text-gray-500 focus:border-[#D4AF37] focus:outline-none transition resize-none">{{ old('description', $research->description) }}</textarea>
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="content" name="content" rows="5"
                    class="w-full px-4 py-2.5 bg-[#0b0e12] border border-white/10 rounded-lg text-white placeholder:text-gray-500 focus:border-[#D4AF37] focus:outline-none transition resize-none">{{ old('content', $research->content) }}</textarea>
            ...
            <div>
                ...
                @if($research->image_url)
                    <div class="mb-2">
                        <img src="{{ $research->image_url }}" alt="{{ $research->title }}" class="w-24 h-24 object-cover rounded-lg border border-white/10">
                    </div>
                @endif
                <div class="relative">
            ...
            <div>
                ...
                <input type="url" id="link_url" name="link_url" value="{{ old('link_url', $research->link_url) }}"
                    class="w-full px-4 py-2.5 bg-[#0b0e12] border border-white/10 rounded-lg text-white placeholder:text-gray-500 focus:border-[#D4AF37] focus:outline-none transition"
            ...
            <div>
                ...
                <input type="url" id="video_url" name="video_url" value="{{ old('video_url', $research->video_url) }}"
                    class="w-full px-4 py-2.5 bg-[#0b0e12] border border-white/10 rounded-lg text-white placeholder:text-gray-500 focus:border-[#D4AF37] focus:outline-none transition"
            ...
            <div>
                ...
                @if($research->video_file)
                    <div class="mb-2 text-sm text-gray-400">
                        <i class="fas fa-file-video text-[#D4AF37] mr-1"></i>
                        Current: {{ basename($research->video_file) }}
                    </div>
                @endif
                <div class="relative">
        ...
        <div class="mt-6 pt-6 border-t border-white/10">
            <label class="flex items-center cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $research->is_active) ? 'checked' : '' }}
        ...
        <div class="mt-4">
            <label class="flex items-center cursor-pointer">
                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $research->is_featured) ? 'checked' : '' }}
                    class="w-4 h-4 rounded border-white/10 bg-[#0b0e12] text-[#D4AF37] focus:ring-[#D4AF37] focus:ring-offset-0 transition">
                <span class="ml-2 text-sm text-gray-300">
        ...
        <div class="mt-6 flex flex-wrap gap-4">
            <button type="submit" class="px-6 py-2.5 bg-[#D4AF37] text-[#0b0e12] rounded-lg hover:bg-[#c4a030] transition flex items-center gap-2 font-medium">
                <i class="fas fa-save"></i> Update Research Item
            </button>
