...
@section('title', 'Create Stat · Admin')
...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-3xl font-serif font-bold text-white">Create New Stat</h1>
    </div>
    ...
    <div class="bg-[#1a1f26] rounded-xl border border-white/5 p-6">
        <form action="{{ route('admin.stats.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="sub_label" name="sub_label" value="{{ old('sub_label') }}"
                    class="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-white focus:border-[#D4AF37] focus:outline-none transition">
                @error('sub_label')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                <label for="icon" class="block text-white/70 text-sm font-medium mb-2">Icon (SVG path or class)</label>
